...
export default function MapAnalysisOverlay(): React.JSX.Element {
  ...
  React.useEffect(() => {
    ...
    return () =>
      window.removeEventListener(
        "sunsettings:pinSelected",
        onPin as EventListener
      );
  }, []);
  ...
  React.useEffect(() => {
    ...
    async function loadMeta() {
      try {
        ...
        const res = await fetch(`${gw}/ipfs/${selectedPin.metadataCid}`, {
          cache: "force-cache",
        });
        const raw: unknown = await res.json().catch(() => null);
        if (aborted || !raw || typeof raw !== "object") {
          setMetaScores(null);
          return;
        }
            ...
            <div className="mt-2 flex items-center justify-between text-xs gap-3">
              <div className="flex min-w-0 flex-col">
                ...
                <span className="font-semibold text-[12px] leading-tight truncate">
                  {typeof scorePercent === "number"
                    ...
                    : "—"}
                  {scoreLabel ? (
                    <span className="opacity-70"> ({String(scoreLabel)})</span>
                  ) : null}
                </span>
              </div>
              <div className="flex flex-col text-right">
                ...
                <span className="font-semibold text-[12px] leading-tight">
                  {typeof userScorePercent === "number"
                    ? `${Math.round(userScorePercent)}%`
                    : "—"}
                  {userScoreLabel ? (
  ...
  const buildWeatherFeatures = React.useCallback(
    async (
      lat: number,
      lon: number
    ): Promise<{
  ...
  React.useEffect(() => {
    ...
    debounceRef.current = window.setTimeout(async () => {
      ...
      inFlightRef.current = (async () => {
        try {
          const wf = await buildWeatherFeatures(
            latNum as number,
            lonNum as number
          );
  ...
  }, [latNum, lonNum, dayBump, buildWeatherFeatures]);
        ...
        <AlertDialog open={locationMismatch} onOpenChange={setLocationMismatch}>
          <AlertDialogContent className="relative">
            ...
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Locations of the sunset forecast and your current device
                location differ. Please re-run the forecast for your current
                location before taking a photo.
              </AlertDialogDescription>
            </AlertDialogHeader>
              ...
              <AlertDialogAction
                onClick={async () => {
                  ...
                  try {
                    ...
                    setGeoError(
                      (e as Error)?.message || "Failed to detect location"
                    );
                  } finally {
                    setGeoLoading(false);
                  }
