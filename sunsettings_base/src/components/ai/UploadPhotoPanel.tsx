...
async function mintPhotoWithWallet(
  ...
  const txHash = await walletClient.writeContract({
    ...
    abi: NFT_MINT_ABI,
    functionName: NFT_MINT_FUNCTION,
    args: [account, tokenUri],
    account,
    chain: walletClient.chain ?? ACTIVE_CHAIN,
  });
...
export default function UploadPhotoPanel({
  ...
  const [mintTxHash, setMintTxHash] = React.useState<Hash | null>(null);
                ...
                <div className="text-sm">Photo pinned to IPFS</div>
                  ...
                  >
                    ...
                    <div className="text-xs opacity-80">
                      Minting NFT on Base Sepolia… confirm the transaction in your wallet.
                  </div>
                ) : mintError ? (
                  ...
                  <div className="pt-2 flex justify-center gap-2">
                    ...
                    {!connectedAddress && (
                      <div className="text-[11px] text-red-600">
                        Connect your Base wallet to submit and mint an NFT.
                      </div>
                    )}
                </div>
              )
  ...
  async function onUpload() {
    ...
    if (chainId !== undefined && chainId !== ACTIVE_CHAIN.id) {
      setError("Switch your wallet to Base Sepolia before uploading");
      return;
    }
    const accountAddress = connectedAddress as Address;
    const activeWalletClient = walletClient as WalletClient;
    setUploading(true);
    ...
    try {
...

      {
        ...
        const metadata = {
          ...
          gpsAccuracy:
            typeof gpsFix?.accuracy === "number" ? gpsFix.accuracy : 0,
          captureTimestamp: captureTimestamp || "",
          prehashSha256: prehashSha256 || "",
          sunsetScorePercent:
            typeof scorePercent === "number" ? scorePercent : null,
          sunsetScoreLabel: scoreLabel || null,
          userSunsetScorePercent:
            typeof userScore === "number" ? userScore : null,
          photoCreatedAt,
        };
        ...
        try {
          ...
          setMinting(true);
          setMintTxHash(null);
          const { txHash, account } = await mintPhotoWithWallet(
            metaJson.cid,
            accountAddress,
            activeWalletClient
          );
          ...
          const raw = (mintErr as Error)?.message || "Mint failed";
          setMintError(raw);
          setMintTxHash(null);
          const lifted = raw.toLowerCase().startsWith("mint")
            ? raw
            : `Mint failed: ${raw}`;
          throw new Error(lifted);
        } finally {
          setMinting(false);
        }
      }
